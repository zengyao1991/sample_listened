...
struct send_mess *pack_data(const char *data, uint32_t cmd){
    ...
    if (data != NULL){
        ...
        struct send_mess *mess = (send_mess*)malloc(sizeof(send_mess));
        mess->cmd = htonl(cmd);
        mess->length = htonl(0);
        return mess;
    }
...
/* > 发送心跳okok */
static void *send_heart(void *cli_fd){
    ...
    while (1) {
        ...
        int cmd = HEART_BEAT;
        const char *data_beat = NULL;
 
        struct send_mess *beat_mess = pack_data(data_beat, cmd);
        int bfd = *fd;
        if((send(bfd, (char *)beat_mess, sizeof(send_mess),0)) < 0){
            printf("beat send error\n");
        }
        free(beat_mess);
        beat_mess = NULL;

        char recv_mess[200];
        if(recv(bfd, recv_mess, sizeof(recv_mess), 0) < 0){
            printf("beat recv error\n");
        }else
            printf("beat :::: %s, %d, len = %d\n", recv_mess, beat_flag++, strlen(recv_mess));    
...
static void *to_work(){
    ...
    //创建心跳线程
    int *beat_fd = (int *)malloc(sizeof(int));
    *beat_fd = sock_cli;
    pthread_t pth;
    if((pthread_create(&pth, NULL, send_heart, (void *)beat_fd)) != 0){
