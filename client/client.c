...
static void *client_to_work();
static void client_create_worker(void *(*func)(void *));

int main(int argc,char** argv)
{
    if (argc < 1) {
        fprintf(stderr, "need num of thread");
        return -1;
    }
    ...
    int i = 0;
    for (i = 0; i < number; i++) {
        client_create_worker(client_to_work);
    }
...
static void *client_to_work()
{
    ...
    if (conn->fd < 0) {
        fprintf(stderr, "fd create err, [%d]\n", errno);
        return NULL;
    }
